  ...
  <section class="ContentHeight">
    ...
    <div class="container-full">
      <div class="bg-dark py-3">
        ...
        <p class="text-white text-center">ثبت نوبت دهي تعميرگاه</p>
    ...
    <div style="min-height: 100vh" class="px-4 pt-4">
      ...
      <div class="container pt-4">
        ...
        <!-- choose service -->
        <select
          @change="SetServiceType"
          id="HomeService"
          class="form-select mt-4">
          <option>سرويس درخواستي</option>
          <option v-for="service in DspBookingType.Data" 
            :key="service.BookingTypeId"
            :value="service.BookingTypeId">
      ...
      <div v-for="Dealer in DspBookingCapacities.Data" :key="Dealer.Id" class="container shadow ReqStatus mt-4 py-3 ps-3">
        ...
        <div class="d-flex align-items-center justify-content-between">
         <!-- choose dealer title -->
          <div>
              <input :id="Dealer.Id" v-model="DealerId" type="radio" :value="Dealer.Id" />
              <label :for="Dealer.Id" class="ChooseTxt px-2">انتخاب عامليت</label>
          </div>
         <!-- choose your time -->
          <select @change="StoreDealerDateId" id="city" class="TheStyle mt-4">
              <option>تاريخ</option>
              <option v-for="Data in Dealer.BookingCapacityInfo" 
              :key="Data.BookingCapacityPortTypeId" 
              :value="Data.BookingCapacityPortTypeId">
                <p>{{ Data.PersianDate }}</p>
              </option>
          </select>
        </div>
      </div>
      ...
      <div class="container">
        ...
        <div class="d-flex justify-content-start mt-3 align-items-center">
          ...
          <div>
            <label for="condition" class="AgreeTxt px-2">شرايط و قوانين فوق را مي پذيرم</label>
          </div>
        </div>
        <!-- register request -->
        <button @click="InsertTheReserve" type="button" class="py-3 BtnRegStyle mt-5 btn">
          ثبت درخواست
        </button>
        <!-- HomeService Success Alert -->
        <div v-if="DspInfo && DspReserve.ResultStatus == '200'" class="container alert alert-success ResultPopUp mt-4 shadow-lg text-center">
          <!-- message content -->
          <p class="text-center">{{ DspReserve.Message }}</p>
          <!-- End message content -->
        ...
        <!-- HomeService Error Alert -->
          <div v-if="DspInfo && DspReserve.ResultStatus == '500'" class="container alert alert-success ResultPopUp mt-4 shadow-lg text-center">
          <!-- message content -->
          <p class="text-center">{{ DspReserve.Message }}</p>
          <!-- End message content -->
          <button class="btn btn-sm btn-primary text-white mt-4 ViewStyle" @click="HideInfo">
...
export default {
  ...
  methods: {
    ...
    InsertTheReserve() {
      ...
      if(this.DealerId && this.DealerDateId && 
         this.deliverername && this.productSerialId &&
         this.nationalId && this.deliverermobile && this.ServiceTypeId ) {
          if(this.Terms) {
                /* load loading */ 
                ...
                this.InsertReserve({
                  ...
                  BookingCapacityPortTypeId: this.DealerDateId, 
                  CustomerName: this.deliverername, 
                  CustomerFamily: this.deliverername, 
                  VIN: this.productSerialId, 
                  NationalityID: this.nationalId,
                  /* haghighi passes here */  
                  LawTypeId: 1, 
                  MobileNo: this.deliverermobile, 
                  BookingTypeId: this.ServiceTypeId
                })
          } else {alert('لطفا قوانين و شرايط را بپذيريد')}
    ...
    HideInfo() {
      this.DspInfo = false;
      this.SetClearReservationStatus();
      this.DspInfo = true;
